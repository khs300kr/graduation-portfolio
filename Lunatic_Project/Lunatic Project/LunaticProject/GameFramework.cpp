		...
		if ((dwDirection != 0) || (cxDelta != 0.0f) || (cyDelta != 0.0f))
		{
			...
			if (dwDirection && ChangeScene == GAME)
			{
				...
				for (int i = 0; i < MAX_GAMER; ++i)
				{
					if (i != m_pScene->myGame_id)
					{
						if (m_pScene->Rightcollision(m_pScene->pHeroObject[m_pScene->myGame_id], m_pScene->pHeroObject[i], 3.0f, 3.0f, 3.0f, 3.0f))
						{
							...
							//m_pPlayer->SetPosition(D3DXVECTOR3(m_pPlayer->GetPosition().x, m_pPlayer->GetPosition().y, m_pPlayer->GetPosition().z));//m_pPlayer->Move(dwDirection, -m_pScene->pHeroObject[m_pScene->myroom_id]->GetSpeed() - 0.5f, true);
							////m_pScene->pHeroObject[m_pScene->myGame_id]->SetPosition(m_pPlayer->GetPosition());
							//cout << "X : " << m_pPlayer->GetPosition().x << endl;
							//cout << "SPEED : " << m_pScene->pHeroObject[m_pScene->myGame_id]->GetSpeed() << endl;
							break;
						}
				...
				for (int i = 1; i < 13; ++i) // house1 <-> Hero 건물 충돌체크 // 아래, 위 해야함
				{
					...
					//else if (m_pScene->Upcollision(m_pScene->pHeroObject[m_pScene->myGame_id], m_pScene->pHouse1Object[i], 3.0f, 3.0f, 50.0f, 45.0f))
					//{
					//	m_pPlayer->SetPosition(D3DXVECTOR3(m_pPlayer->GetPosition().x, m_pPlayer->GetPosition().y, m_pPlayer->GetPosition().z + m_pScene->pHeroObject[m_pScene->myGame_id]->GetSpeed()));//m_pPlayer->Move(dwDirection, -m_pScene->pHeroObject[m_pScene->myroom_id]->GetSpeed() - 0.5f, true);
					//	m_pScene->pHeroObject[m_pScene->myGame_id]->SetPosition(m_pPlayer->GetPosition());
					//	//char_collision_send();
					//	break;
					//}
					//else if (m_pScene->Downcollision(m_pScene->pHeroObject[m_pScene->myGame_id], m_pScene->pHouse1Object[i], 3.0f, 3.0f, 50.0f, 45.0f))
					//{
					//	m_pPlayer->SetPosition(D3DXVECTOR3(m_pPlayer->GetPosition().x, m_pPlayer->GetPosition().y, m_pPlayer->GetPosition().z - m_pScene->pHeroObject[m_pScene->myGame_id]->GetSpeed()));//m_pPlayer->Move(dwDirection, -m_pScene->pHeroObject[m_pScene->myroom_id]->GetSpeed() - 0.5f, true);
					//	m_pScene->pHeroObject[m_pScene->myGame_id]->SetPosition(m_pPlayer->GetPosition());
					//	//char_collision_send();
