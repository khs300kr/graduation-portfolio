...
void ProcessPacket(char * ptr)
{
	...
	case SC_PUT_PLAYER:
	{
		...
		if (id == g_myid) {
			...
			cout << "Other Pos\n";
			gGameFramework.m_pScene->pOtherObject[id]->SetPosition(my_packet->x, my_packet->y, my_packet->z + 20.f);
		}
		break;
	}
	case SC_POS:
	{
		...
		if (id == g_myid) {
			cout << "Hero Move\n";
			gGameFramework.dwDirection = my_packet->dwDirection;
			if(my_packet->dwDirection!= 0)
			...
			cout << "Other Move\n";
			gGameFramework.OtherDirection[id] = my_packet->dwDirection;
			if (my_packet->dwDirection != 0)
